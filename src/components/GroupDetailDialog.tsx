...
const GroupDetailDialog = ({
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl border border-gray-200 dark:border-gray-700 bg-neutral-900">
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            {!isJoined && <Button onClick={handleJoinClick} disabled={group.memberIds.length >= group.memberLimit} className="flex-1">
                {group.memberIds.length >= group.memberLimit ? 'Group Full' : 'Request to Join'}
              </Button>}
            
            {isJoined && <Button variant="secondary" className="flex-1 dark:bg-gray-700 dark:text-gray-300" disabled>
                Already Joined
              </Button>}
          </div>
        </div>
